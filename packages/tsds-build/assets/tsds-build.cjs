(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('path'), require('queue-cb'), require('rimraf2'), require('mkdirp'), require('resolve'), require('url'), require('cross-spawn-cb'), require('env-path-key'), require('path-string-prepend'), require('fs'), require('ts-swc-transform'), require('fs-iterator'), require('get-tsconfig-compat')) :
    typeof define === 'function' && define.amd ? define(['path', 'queue-cb', 'rimraf2', 'mkdirp', 'resolve', 'url', 'cross-spawn-cb', 'env-path-key', 'path-string-prepend', 'fs', 'ts-swc-transform', 'fs-iterator', 'get-tsconfig-compat'], factory) :
...
})(this, (function (path, Queue, rimraf2, mkdirp, resolve, url, crossSpawn, pathKey, prepend, fs, tsSwcTransform, Iterator, getTS) { 'use strict';
    ...
    function spawn(cmd, args, options, cb) {
        ...
        env[PATH_KEY] = prepend(env[PATH_KEY] || '', path.resolve(__dirname$2, '..', '..', '..', '..', '..', 'node_modules', '.bin'));
