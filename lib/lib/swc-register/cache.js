...
function Cache(options) {
  ...
  this.cwdHash = this.hashString(process.cwd());
  this.root = options.root || path.join(tmpdir(), 'swc-register');
...
Cache.prototype.cachePath = function (filePath, options) {
  ...
  var dirHash = this.hashString(path.dirname(relFilePath));
  if (options) basename += '-' + this.hashString(JSON.stringify(options || {}));
  return path.join(this.root, this.cwdHash, dirHash, basename + '.json');
};

Cache.prototype.get = function (cachePath) {
  var record = getRecord(cachePath)
...
Cache.prototype.getOrUpdate = function getOrUpdate(cachePath, contents, fn) {
  var hash = this.hashString(contents);
...
Cache.prototype.hashString = function hashString(string) {
  return shortHash(string);
};
