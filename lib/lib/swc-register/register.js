...
var readConfig = path.join(__dirname, 'readConfig.js');
var transformSync = path.join(__dirname, 'transformSync.js');
...
function register(options, hookOpts) {
  ...
  if (!options.tsConfig) {
    ...
    options.tsConfig = call(version, readConfig, filename);
...
function compile(contents, filename, options) {
  ...
  var data = cache.getOrUpdate(cache.cachePath(filename, options.tsConfig.options), contents, function () {
    return call(version, transformSync, contents, filename, options.tsConfig.options);
  });
  return data.code;
}
