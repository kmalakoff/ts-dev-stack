...
function saveLink(installPath, cb) {
  ...
  queue.defer(fs.rename.bind(null, installPath, 'tsds.' + installPath));
  queue.defer(createLink.bind(null, installPath));
  queue.await(cb);
}
function restoreLink(installPath, cb) {
  var queue = new Queue(1);
  queue.defer(fs.rename.bind(null, 'tsds.' + installPath, installPath));
    ...
    fs.lstat(installPath, function (_, lstat) {
      ...
      // exists so move it
        ...
        saveLink(installPath, function (err) {
          err ? cb(err) : cb(null, restoreLink.bind(null, installPath));
        });
      }
